...
class NodeLessTest {
    ...
    void evaluationTest() throws ASTOperationException {
        NodeLess node = new NodeLess(new NodeExternal(randomSInt1), new NodeExternal(randomSInt2));
        assertEquals(new SBool(randomInt1 < randomInt2), node.evaluate());
    }

    @Test
    void exceptionTest() {
        NodeLess node = new NodeLess(new NodeExternal(randomSInt1), new NodeExternal(new SString("nya")));
