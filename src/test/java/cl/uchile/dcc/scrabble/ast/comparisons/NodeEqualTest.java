...
class NodeEqualTest {
    ...
    void evaluationTrueTest() throws ASTOperationException {
        NodeEqual node = new NodeEqual(new NodeExternal(randomSInt1), new NodeExternal(randomSInt1));
        assertEquals(new SBool(true), node.evaluate());
    }

    @RepeatedTest(value=20, name=RepeatedTest.LONG_DISPLAY_NAME)
    void evaluationTest() throws ASTOperationException {
        NodeEqual node = new NodeEqual(new NodeExternal(randomSInt1), new NodeExternal(randomSInt2));
