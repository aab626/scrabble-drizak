...
class NodeNotEqualTest {
    ...
    void evaluationFalseTest() throws ASTOperationException {
        NodeNotEqual node = new NodeNotEqual(new NodeExternal(randomSInt1), new NodeExternal(randomSInt1));
        assertEquals(new SBool(false), node.evaluate());
    }

    @RepeatedTest(value=20, name=RepeatedTest.LONG_DISPLAY_NAME)
    void evaluationTest() throws ASTOperationException {
        NodeNotEqual node = new NodeNotEqual(new NodeExternal(randomSInt1), new NodeExternal(randomSInt2));
